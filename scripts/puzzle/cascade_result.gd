...
## Phase R-D: cascade resolution outcome.
...
static func guard_exceeded(step_count: int, cleared_per_step: Array) -> CascadeResult:
	var result := CascadeResult.new()
	result._valid = false
	result._stable = false
...
static func stable_success(step_count: int, cleared_per_step: Array) -> CascadeResult:
	var result := CascadeResult.new()
	result._valid = true
	result._stable = true
	result._guard_exceeded = false
